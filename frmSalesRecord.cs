...
{
    public partial class frmSalesRecord : MetroFramework.Forms.MetroForm
    {
    ...
    ConnectionString cs = new ConnectionString();
        ...
        private void Button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = new SqlConnection(cs.DBConn);
                con.Open();
                cmd = new SqlCommand("SELECT RTRIM(invoiceNo) as [Order No.],RTRIM(InvoiceDate) as [Order Date],RTRIM(SubTotal) as [SubTotal],RTRIM(VATPer) as [Vat+ST %],RTRIM(VATAmount) as [VAT+ST Amount],RTRIM(DiscountPer) as [Discount %],RTRIM(DiscountAmount) as [Discount Amount],RTRIM(GrandTotal) as [Grand Total],RTRIM(TotalPayment) as [Total Payment],RTRIM(PaymentDue) as [Payment Due],Remarks from Invoice_Info where InvoiceDate between @d1 and @d2 and PaymentDue > 0 order by InvoiceDate desc", con);
